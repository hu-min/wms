...
class FinanceController extends BaseController{
    ...
    function wouldpay_modalOne(){
        ...
        $resultData['suprpaidItem'] = $this->fetch('Finance/financeTable/suprpaidItem');
    ...
    function getReckonList($return=false){
        $sql = "SELECT pc.project_id project_id,pc.section section,CASE WHEN ISNULL(vp.name) THEN '非项目' ELSE vp.name END `name` ,CASE WHEN ISNULL(vp.code) THEN '非项目' ELSE vp.code END `code`,CASE WHEN ISNULL(flag) THEN '非项目' ELSE flag END flag,d.debit_money debit_money,d.debit_num debit_num,e.expense_money expense_money,e.expense_num expense_num,e.invoice_money invoice_money FROM (SELECT project_id,section FROM v_debit WHERE `status`=1 AND user_id=".session("userId")." AND clear_status = 0 GROUP BY project_id, section UNION SELECT project_id,section FROM v_expense WHERE `status`=1 AND user_id=".session("userId")." AND clear_status = 0 GROUP BY project_id, section) ps LEFT JOIN (SELECT project_id,section,SUM(debit_money) debit_money,COUNT(id) debit_num FROM v_debit WHERE `status`=1 AND user_id=".session("userId")." AND clear_status = 0 GROUP BY project_id, section) d ON d.project_id = ps.project_id AND d.section = ps.section LEFT JOIN (SELECT project_id,section,SUM(money) expense_money,COUNT(id) expense_num,SUM(invoice_money) invoice_money FROM v_expense WHERE `status`=1 AND user_id=".session("userId")." AND clear_status = 0 GROUP BY project_id, section) e ON e.project_id = ps.project_id AND e.section = ps.section LEFT JOIN (SELECT projectId,name,code FROM v_project) vp ON vp.projectId=ps.project_id LEFT JOIN (SELECT project_id,section,flag FROM v_project_cost) pc ON pc.project_id=ps.project_id AND pc.section = ps.section";
    ...
    function flo_cap_logList(){
        ...
        $parameter=[
            ...
            "joins"=>[
                ...
                "LEFT JOIN (SELECT id approve_id,table_id FROM v_approve_log WHERE table_name='".$this->flCapLogCom."' AND status > 0 AND user_id = '".session("userId")."' AND effect = 1 ) a ON a.table_id = id"
            ],
        ];
    ...
    function getSectionList(){
        ...
        $param = [
            ...
            'isCount' =>false,
            'joins'=>[
                'RIGHT JOIN (SELECT parent_cid,scompany_id FROM v_project_cost_sub WHERE scompany_id > 0) pcs ON pcs.parent_cid = id'
            ]
        ];
