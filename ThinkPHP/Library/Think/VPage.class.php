...
class VPage{
    ...
    public function show($onlyPage=false) {
        ...
        if(!empty($this->totalPages) && $this->nowPage > $this->totalPages) {
            ...
            $this->firstRow = $this->listRows * ($this->nowPage - 1);   // 修复页数大于总页数不显示数据的BUG
        }
        ...
        $up_page = $up_row > 0 ? '<li class="paginate_button previous" id="example2_previous"><a href="#" class="vpage" data-page="' .$up_row. '">' . $this->config['prev'] . '</a></li>' : '';

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<li class="paginate_button next" id="example2_next"><a href="#" class="vpage" data-page="' .$down_row. '" >' . $this->config['next'] . '</a></li>' : '';
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page <= $this->totalPages){
                    $link_page .= '<li class="paginate_button "><a href="#" class="vpage" data-page="'.$page.'">' . $page . '</a></li>';
                }else{
                    break;
                }
            }else{
        ...
        //替换分页内容
        $header_str=str_replace(['%HEADER%','%TOTAL_ROW%'],[$this->config['header'],$this->totalRows],$this->config['head']);
