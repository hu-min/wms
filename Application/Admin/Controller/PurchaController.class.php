...
class PurchaController extends BaseController{
    ...
    function purchaApply(){
        ...
        if($reqType){
            ...
            //自动计算成本
            $sql = "SELECT s_parent_cid , s_scompany_id,s_cost_total,s_scompany_cid,item_num,contract_money,wid FROM (SELECT parent_cid s_parent_cid , scompany_id s_scompany_id,SUM(cost_total) s_cost_total, scompany_cid s_scompany_cid,COUNT(id) item_num  FROM v_project_cost_sub RIGHT JOIN (SELECT id cid FROM v_project_cost WHERE `status` = 1) pc ON pc.cid = parent_cid WHERE scompany_id > 0 GROUP BY parent_cid,scompany_id) pcs LEFT JOIN (SELECT id wid , cost_id,supplier_id,contract_money FROM v_wouldpay) w ON w.cost_id = pcs.s_parent_cid AND w.supplier_id = pcs.s_scompany_id";
            // $this->log();
            ...
            foreach ($listResult as  $supplierCost) {
                ...
                }elseif($supplierCost['s_cost_total'] != $supplierCost['contract_money']){
                    $insertResult = $this->wouldpayCom->update(['where'=>['id'=>$supplierCost['wid']],'data'=>['cost_id'=>$supplierCost['s_parent_cid'],'supplier_id'=>$supplierCost['s_scompany_id'],'contract_money'=>$supplierCost['s_cost_total'],'add_time'=>time(),'status'=>1,'supplier_cid'=>$supplierCost['s_scompany_cid'],'item_num'=>$supplierCost['item_num']]]);
                }
    ...
    function purcha_applyEdit(){
        ...
        if(isset($dels)){
            $delResult = $this->payCom->delete(implode(',',$dels));
        }
        if($isUpdate || (isset($delResult) && $delResult)){
            $this->ajaxReturn(['errCode'=>0,'error'=>"修改成功"]);
        }
        $this->ajaxReturn(['errCode'=>$updateResult->errCode,'error'=>$updateResult->error]);
    }
